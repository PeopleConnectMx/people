...
class AdminController extends Controller {
    ...
    public function Index() {
        ...
        return view('admin.nuevoEmpleado', compact('super', 'Reclutador', 'analistaCalidad', 'teamLeader', 'coor', 'coach', 'area', 'puesto', 'camp', 'menu'));
    }
    ...
    public function NewEmpleado(Request $request) {
        ...
        $empleado->user_ext = trim($request->user_ext);
        ...
        $histEmple->user_ext = trim($request->user_ext);
        ...
        $histo_candi = new historico_candidatos;
        $histo_candi->id_num_emp = $noE;
        $histo_candi->nombre_completo = $nom_completo;
        $histo_candi->paterno = $request->paterno;
        $histo_candi->materno = $request->materno;
        $histo_candi->nombre = $request->nombre;
        $histo_candi->turno = $request->turno;
        $histo_candi->area = $request->area;
        $histo_candi->puesto = $request->puesto;
        $histo_candi->sucursal = $request->sucursal;
        $histo_candi->estadoCandidato = 'Aceptado';
        $histo_candi->campaign = $request->campaign;
        $histo_candi->ejec_entrevista = $request->ejecReclutamiento;
        $histo_candi->tipo_medio_reclutamiento = $request->tipoMedioReclutamiento;
        $histo_candi->tipo_contrato = $request->tipo_contrato;

        if (!empty($request->medioReclutamiento)) {
            $histo_candi->medio_reclutamiento = $request->medioReclutamiento;
        } else {
            $histo_candi->medio_reclutamiento = '';
        }
        $histo_candi->fecha_capacitacion = $request->fecha_ingreso_capacitacion;
        $histo_candi->movimiento = $user['user'];
        $histo_candi->save();
    ...
    public function DownEmpleado($id = '') {
        ...
        if (!empty($empleadoVal)) {
            $histEliminado->user_ext = trim($empleadoVal[0]->user_ext);
    ...
    public function UpEmpleado(Request $request) {
        ...
        $emp->user_ext = trim($request->user_ext);
        ...
        $DetalleEmpleado->forma_pago = $request->forma_pago;
        $DetalleEmpleado->tipo_cuenta = $request->tipo_cuenta;
        $DetalleEmpleado->num_cuenta = $request->num_cuenta;
        ...
        $histCandida->nombre_completo = $nom_completo;
        ...
        $histCandida->fecha_capacitacion = $request->fecha_ingreso_capacitacion;
    ...
    public function ShowUser($value = "") {
        ...
        if (DetalleEmpleado::find($value)) {
            $DetalleEmpleado = DB::table('detalle_empleados')
                    ->select('imssPlan','imssFact','motivoBaja','teamLeader',
                    'analistaCalidad', 'usuarioAuxiliar', 'posicion', 'coach', 'correo_administrativo',
                    'forma_pago','tipo_cuenta','num_cuenta'
                    )
                    ->where('id', $value)
                    ->get();
        } else {
                    ...
                    case 'Administracion':
                        switch ($puesto) {
                            ...
                            case 'Capturista':
                                ...
                                return $coor;
                                break;
                            case 'Auditor de procesos':
                                $coor = DB::table('empleados')
                                        ->select('usuarios.id', 'candidatos.nombre_completo')
                                        ->join('usuarios', 'usuarios.id', '=', 'empleados.id')
                                        ->join('candidatos', 'candidatos.id', '=', 'usuarios.id')
                                        ->where(['usuarios.active' => true, 'candidatos.puesto' => 'Jefe de procesos', 'candidatos.area' => 'Administracion'])
                                        ->orderBy('nombre_completo', 'asc')
                                        ->get();
                                return $coor;
                                break;
