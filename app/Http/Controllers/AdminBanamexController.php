...
class AdminBanamexController extends Controller {
...
$hora++;
      ...
      Excel::create('Reporte general conaliteg '.date('d-m-Y'), function ($libro) use($dt,$medioDiaCabData,$tituloDiaCabData){
        ...
        $libro->sheet('Reporte por dia título', function($h) use($tituloDiaCabData) { $h->fromArray($tituloDiaCabData); });
    ...
    public function Index() {
        ...
        return view('Banamex.admin.nuevoEmpleado', compact('super','Reclutador','analistaCalidad','teamLeader','coor'));
    }
    ...
    public function NewEmpleado(Request $request) {
        ...
        return View('admin.confirm', ['id' => $noE, 'nombre' => $nom_completo, 'mensaje' => '0']);
    }
    ...
    public function GetUsers() {
      ...
      switch ($puesto) {
        ...
        #return view('admin.plantilla', compact('menu', 'users'));
    }
    ...
    public function ShowUser($value = "") {
      ...
      $identificador=false;
        ...
        //using pagination method
        return view('admin.updateEmpleado', compact('user','super','coor','datosCandidato','DetalleEmpleado','Reclutador','analistaCalidad','teamLeader','usuario','menu'));
    }
...
public function ReporteAsistenciaHistorico(Request $request) {
        ...
        $excel->sheet('asistencia', function($sheet) use($request) {
                ...
                $top=array("Empleado","Nombre Completo","Supervisor","Area","Puesto","Campaña","Turno","Fecha de Ingreso","Estatus");
                        ...
                        /*$datos = DB::table('candidatos as ca')
                                ->select('ca.id', 'ca.nombre', 'ca.paterno', 'ca.materno', 'emp.nombre_completo', 'ca.area', 'ca.puesto', 'ca.campaign', 'ca.turno', 'ca.fecha_capacitacion', DB::raw("if(us.active=true,'Activo','Inactivo') as estatus"), 'hi.dia', 'hi.record' )
                                ->join('usuarios as us', 'us.id', '=', 'ca.id' )
                                ->join('empleados as em', 'em.id', '=', 'us.id')
                                ->leftjoin('empleados as emp', 'emp.id', '=', 'em.supervisor')
                                ->join('historico_asistencias as hi', 'em.id', '=', 'hi.usuario' )
                                ->where(['us.active'=>true])
                                ->whereBetween('hi.dia', [$request->inicio, $request->fin])
                                ->get();*/
                         $empleados=DB::table('candidatos')
...
function ValiAcento($string)
 {
    #$datos= Plantilla::find($request->$cadena);
