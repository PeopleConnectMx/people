...
class BancomerController extends Controller {
    ...
    public function Audio($dn = '', $fecha = '') {
        ...
        #$location = "file://192.168.10.90/Inbursa/$anio/$mes/$dia"; /*serv 10*/
        try {
            $location = file_get_contents("http://192.168.10.90/Gral/Inbursa/$anio/$mes/$dia", 'r');
            $location = explode("\n", $location);
    ...
    public function fechaAudios(){
        return view('Bancomer.edicion.fechaEdicion');
    }

    public function DatosVentas(Request $value){
        $datos = DB::table('bancomer_3.tipificacion')
            ->select('dn', 'b_id', 'v_id', 'status', 'fecha')
            ->where([['status', '=', 'Encuesta efectiva'], ['fecha', '=', $value->fecha]])
            ->get();

        return view('Bancomer.edicion.listaAudios', compact('datos'));
    }

    public function Audios($telefono, $fecha, $id){
        ...
        return view('Bancomer.edicion.descarga', compact('anio', 'mes', 'dia', 'audios', 'telefono'));
    }


    function inicioBuscaAudio($folio) {
        ...
        try {
          ...
          ->where('v_id',$folio)
          ->get();
          $fecha=str_replace("-", "/", $datos[0]->fecha);
          $url = "http://201.168.130.213:256/bbva-bmx/Bancomer/".$fecha."/".$datos[0]->nombre_audio.".wav";
          $filename = $folio.'.wav';
    ...
    /*function descarga(Request $param) {
        #dd(date('m', strtotime($param->fecha)), date('d', strtotime($param->fecha)));

        $datos = $this->findfile(date('Y', strtotime($param->fecha)), date('m', strtotime($param->fecha)), date('d', strtotime($param->fecha)), $param->dn);
    ...
    function findfile($anio, $mes, $dia, $telefono) {
        ...
        try {
            $location = file_get_contents("http://201.168.130.213:256/bbva-bmx/Bancomer/$anio/$mes/$dia", 'r');
            $location = explode("\n", $location);
