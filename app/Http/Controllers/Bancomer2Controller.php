...
use App\Model\Bancomer2\Tipificacion;
use League\Flysystem\Filesystem;

class Bancomer2Controller extends Controller {

    public function Inicio() {
        $menu = $this->Menu();
        return view('Bancomer2.Agente.inicio', compact('menu'));
    }
    ...
    public function Guarda(Request $request) {
      //dd('o');
        ...
        $fol = $this->Folio();
        $datos = DB::table('bancomer_2.datos')
        ...
        if ($request->tipificacion == 'Encuesta efectiva') {
          ...
          // }
            $val = 1;
            return redirect('/Bancomer_2/guardar/registro/' . $fol . '/' . $val);
        } else {
            $val = 0;
            return redirect('/Bancomer_2/guardar/registro/' . $fol . '/' . $val);
        }
    ...
    public function Confirm($fol = '', $val = '') {
        $menu = $this->Menu();
        return view('Bancomer2.Agente.folio', compact('menu', 'fol', 'val'));
    }
    ...
    public function Audio($dn = '', $fecha = '') {
        ...
        $location = "/home/Grabaciones_Bancomer2/$anio/$mes/$dia"; /*serv 10*/
    ...
    public function ListaAudios(){
      return view('Bancomer2.Audios.audios');
    }
    ...
    public function ftp() {
      $datos = DB::table('bancomer_2.tipificacion')
              ...
              ->where(['status' => 'Encuesta efectiva', ['nombre_audio', '<>', ''],'fecha_audio'=>'2017-07-31',[DB::raw("time(created_at)"),'>','15:00:00']])
              ->get();
              $cont=0;
              $x='';
      foreach ($datos as $key => $value) {
        ...
        if($val){
          ...
          Storage::disk('ftp')->put("/audios/Hipotecario/Audios/$value->anio/$value->mes/$value->dia/$value->nombre_audio.wav",$contents1);
    ...
    public function Reportes() {
        $menu = $this->Menu();
        return view('Bancomer2.reportes.reportes', compact('menu'));
    }
    ...
    public function Folio() {
        $hoy = date('Y-m-d');
        $ventas = DB::table('bancomer_2.tipificacion')
                ->select('v_id')
                ->whereDate('created_at', '=', date('Y-m-d'))
                ->where([['v_id', '<>', '']])
                ->count();
        $noVent = DB::table('bancomer_2.tipificacion')
                ->select('v_id')
                ->whereDate('created_at', '=', date('Y-m-d'))
                ->max('v_id');
        $num = substr($noVent, 4);
    ...
    public function Asistencia() {
        $menu = $this->Menu();
        return view('Bancomer2.reportes.asistencia', compact('menu'));
    }
                ...
                switch (Session('puesto')) {
                    case 'Supervisor':
                        switch (Session('campaign')) {
                            case 'Bancomer':
                                $menu = "layout.Bancomer2.Supervisor.supervisor";
                                break;
                        }
                        break;
                    case 'Coordinador':
                        switch (Session('campaign')) {
                            case 'Bancomer':
                                $menu = "layout.Bancomer2.Coordinador.coordinador";
                                break;
                            case 'Banamex':
                    ...
                    case 'Operador de Call Center':
                        switch (Session('campaign')) {
                            case 'Bancomer':
                                $menu = "layout.Bancomer2.Agente.agente";
                                break;
                        }
                        break;
                }
            ...
            case 'Calidad':
                switch (Session('puesto')) {
                    case'Analista de Calidad':
                        switch (Session('campaign')) {
                            case 'Bancomer':
                                $menu = "layout.Bancomer2.Calidad.analista";
                                break;
                        }
                        break;
                }
