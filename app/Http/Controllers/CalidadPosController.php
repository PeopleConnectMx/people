...
use App\Model\TmPospago\pos_calidad_validador;
use App\Model\TmPospago\PosDw;
use DB;
use Session;

class CalidadPosController extends Controller
{
    public function inicio()
    {
        return view('calidad.Pospago.inicio');
    }
        ...
        switch ($request->area) {
            ...
            case 'bo':
                ...
                $datos=DB::table('usuarios as u')
                         ...
                         ->where(['u.active'=>true,'c.puesto'=>'Analista de BO','campaign'=>'TM Pospago','d.analistaCalidad'=>$user['user']])
                         ->get();
                ...
                return view('calidad.Pospago.backoffice.plantilla',compact('datos','fechaValue','fecha_i','fecha_f','moni','numMoni'));
            break;
            ...
            case 'occ':
                ...
                $datos=DB::table('usuarios as u')
                         ...
                         ->join('detalle_empleados as d','d.id','=','c.id')
                         ->leftjoin('empleados as e2','e2.id','=','e.supervisor')
                         ->where(['u.active'=>true,'c.puesto'=>'Operador de Call Center','campaign'=>'TM Pospago','d.analistaCalidad'=>$user['user']])
                         ->get();

                $ConsultaVph=PosDw::select('usuario','fecha_val',DB::raw("count(fecha_val) as total"))
                ...
                return view('calidad.Pospago.plantilla',compact('datosArray','datos','fechaValue','fecha_i','fecha_f','moni','numMoni'));
            break;
            ...
            case 'validador':
                ...
                $datos=DB::table('usuarios as u')
                         ...
                         ->join('detalle_empleados as d','d.id','=','c.id')
                         ->leftjoin('empleados as e2','e2.id','=','e.supervisor')
                         ->where(['u.active'=>true,'c.puesto'=>'Validador','campaign'=>'TM Pospago','d.analistaCalidad'=>$user['user']])
                         ->get();
                ...
                $numMoni=DB::table('calidad_validadors')
                           ...
                           ->groupBy('validador')
                           ->get();
                return view('calidad.Pospago.validador.plantilla',compact('datos','fechaValue','fecha_i','fecha_f','moni','numMoni'));
            break;
    ...
    public function updateValidacion(Request $request){
        ...
        $datosCalidad=pos_calidad_validador::find($request->idValidacion);
        $datosCalidad->dn=$request->dn;
        #$datosCalidad->calidad=$user['user'];
        $datosCalidad->validador=$request->id;
        $datosCalidad->fecha_val=$request->fechaValidacion;
        ...
        return redirect('/calidad/Pospago/validador/NumMon/'.$request->id.'/'.$request->fechaMon);
      }
    public function NoValidado(){
        $date = date( "Y-m-d" );
        $ayer = date( "Y-m-d", strtotime( "-2 day", strtotime( $date ) ) );
        $datos=PosDw::select('pos_dw.dn','fecha',DB::raw("if(pos_dw.dn = pospago.pos_calidad_validador.dn,'Auditado','') as estatus"))
                    ->leftjoin('pospago.pos_calidad_validador','pos_dw.dn','=','pospago.pos_calidad_validador.dn')
                    ->where([['tipificar','<>','Acepta Oferta / NIP'],['tipificar','<>','Acepta Oferta / Nip Modificado'],['cod','like','Trans%']])
                    ->whereBetween('pos_dw.fecha_val',[$ayer,$date])
                    ->get();
        return view('calidad.Pospago.validador.novalidado.dn',compact('datos'));
    }
    public function NoValidadoDatos($dn=''){
        $validadores=DB::table('candidatos as c')
                       ...
                       ->where(['u.active'=>true,'c.area'=>'Validación','c.puesto'=>'Validador','c.campaign'=>'TM Pospago'])
                       ->pluck('nombre_completo','id');
         $supervisor=DB::table('candidatos as c')
                        ->select('c.id','c.nombre_completo')
                        ->join('usuarios as u','u.id','=','c.id')
                        ->where(['u.active'=>true,'c.area'=>'Operaciones','c.puesto'=>'Supervisor','c.campaign'=>'TM Pospago'])
                        ->pluck('nombre_completo','id');
          $analista=DB::table('candidatos as c')
                         ->select('c.id','c.nombre_completo')
                         ->join('usuarios as u','u.id','=','c.id')
                         ->where(['u.active'=>true,'c.area'=>'Calidad','c.puesto'=>'Analista de Calidad','c.campaign'=>'TM Pospago'])
                         ->pluck('nombre_completo','id');

        $valida=pos_calidad_validador::where('dn',$dn)
                  ->get();
        
        /*dd($valida,$validadores,$supervisor,$analista,$dn);*/
        if($valida)
        {
            return view('calidad.Pospago.validador.novalidado.formulario',compact('dn','validadores','supervisor','analista'));
        }
        else
        {
            return view('calidad.Pospago.validador.novalidado.formularioDatos',compact('valida','validadores','supervisor','analista'));
        }
    ...
    public function Auditados(Request $request){
      #dd($request->validador);
        ...
        $datosCalidad=new pos_calidad_validador;
        $datosCalidad->dn = $request->dn;
        $datosCalidad->calidad = session('user');
        ...
        return redirect('/calidad/pospago/novalidado');
    }
    public function AuditadosUpdate(Request $request){
        ...
        $datosCalidad= pos_calidad_validador::find($request->id);
        $datosCalidad->dn=$request->dn;
        #$datosCalidad->calidad=session('user');
        $datosCalidad->validador=$request->validador_f;
        $datosCalidad->imputable=$request->imputable;
    ...
    public function test(){
        return view('calidad.TM_pospago.TMpospa');
    }
    public function mc(){
        return view('calidad.TM_pospago.modcontro');
    }
    public function TMVal(){
        return view('calidad.TM_pospago.TMValida');
    }
    public function TMVal2(){
        return view('calidad.TM_pospago.TMValida2');
    }
    public function Bo(){
        return view('calidad.TM_pospago.TMBO');
    }
    public function us(){
